		  ...
		  <el-container>
			...
			class="border">
				<el-pagination :current-page="page.current" :page-sizes="page.sizes"  :page-size="page.size"
				  layout="prev, next" :total="page.total" @size-change="handleSizeChange"  @current-change="handleCurrentChange">
				</el-pagination>
			</el-footer>
			<el-container>
			  ...
			  <el-main style="position: absolute;top: 60px;left:200px;bottom: 0;right: 0;">
				  <div class="d-flex flex-wrap">
					  ...
					  <span class="border rounded py-1 px-2 m-2"
					     style="cursor: pointer;" v-for="(item,index) in skuItems" :key="index" 
					     :class="item.isCheck ? 'sum-active':''"  @click="choose(item)">
						 {{item.name}}
					  </span>
	...
	export default {
			...
			return {
				...
				skusList:[{
				         name:"颜色",
						  ...
						  list:[{name:"黄色",id:"1",isCheck:false,image:"",color:""},
						        {name:"红色",id:"2",isCheck:false,image:"",color:""}]
			           },{
						  name:"尺寸",
						  type: 0,  // 0 无 1 颜色  2 图片
						  list:[{name:"大号",id:"3",isCheck:false,image:"",color:""},
						        {name:"小号",id:"4",isCheck:false,image:"",color:""}]
			        }],
			}
		},
		computed: {
		...
		methods: {
			...
			choose(item){
				...
				// 之前选中
				// 找到当前对象在chooseList中的索引  获取数组中第一个元素索引位置
